...
csv_path = os.path.join(BASE_DIR, "data", "PART1_NEW_Dallas_Properties.csv")
...
def load_real_estate_data():
    ...
    df["DAYS_ON_MARKET_SAFE"] = 0  # not in this dataset
...
if address_query.strip():
    ...
    addr_norm  = df["ADDRESS"].apply(_normalize_addr)
    mask_exact  = addr_norm == query_norm
    mask_starts = addr_norm.str.startswith(query_norm)
    words = query_norm.split()
    mask_words = pd.Series(True, index=df.index)
    for w in words:
        mask_words &= addr_norm.str.contains(_re.escape(w), na=False)

    if mask_exact.any():
        candidates = df[mask_exact]
    elif mask_starts.any():
        candidates = df[mask_starts]
    else:
        candidates = df[mask_words]

    candidates = candidates.dropna(subset=["LATITUDE", "LONGITUDE"])
    if not candidates.empty:
        searched_property = candidates.iloc[0]
        search_feedback   = "found"
    else:
        search_feedback   = "not_found"

_chart_city = searched_property.get("CITY") if searched_property is not None else target_city
...
trend_data = get_price_trends(df, _chart_city)
roi_data   = get_roi_by_type(df, _chart_city)
...
if search_feedback == "not_found":
    st.markdown(
        ...
        "⚠️ No matching address found in the dataset.</div>",
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        ...
        f"✓ Found: <b>{addr_display}</b>, {city_display} — {price_display}</div>",
        unsafe_allow_html=True,
    )
...
if searched_property is not None:
    _map_city     = searched_property.get("CITY", target_city)
    map_source_df = df[df["CITY"].str.lower() == _map_city.lower()].copy()
...
for _, row in map_df.iterrows():
    ...
    if pd.notna(price_val):
        parts.append(f"Price: <b>${price_val:,.0f}</b>")
...
with map_col:
    ...
    st_folium(fmap, width="100%", height=400, key=f"map_{map_key_sfx}")
...
<div class='dash-section'>
  ...
  <div>
    ...
    <div class='dash-section-title'>Top Performing Listings — {_chart_city}</div>
  </div>
</div>
""", unsafe_allow_html=True)

top_table = get_top_listings(df, _chart_city)
